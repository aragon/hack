...
aragonOS comes with a powerful [ACL](/docs/acl-intro.html) that apps can use for managing permission for calling functions. Rather than coding any custom access control logic into your app, such as the infamous `onlyOwner`, you can just protect functions in your app behind the ACL by adding the `auth` modifier.
